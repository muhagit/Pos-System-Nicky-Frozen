...
const AddUserForm = ({ showModal, setShowModal, selectedUser, onSuccess }) => {
  ...
  const handleSave = async () => {
    ...
    // Simulasi proses simpan (ganti dengan API call)
    // Di sini Anda bisa panggil API atau update state
    try {
      ...
      setShowModal(false);
      if (onSuccess) onSuccess(); // Refresh data atau callback
    } catch (error) {
      Swal.fire({
        icon: "error",
        title: "Failed",
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-card w-full max-w-2xl rounded-3xl p-6 border border-border">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-text">Add User</h2>
